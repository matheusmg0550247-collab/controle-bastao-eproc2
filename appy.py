...
def generate_app_code(consultores, emoji, webhook_relatorio, webhook_bastao, public_url):
    ...
    app_code_lines = [
        ...
        "    # save_state() is called by the main action functions",
        "",
        ...
        "        # Set default based on whether they are in active queue OR skipped",
        "        # OR if they have a non-empty status_texto (implies they were active)",
        ...
        "",
        "    # Identify who *should* complete this lap (available and not skipped)",
        "    should_complete = set()",
        ...
        "",
        "    # Reset if the 'should_complete' set is not empty and is a subset of 'completed'",
        ...
        "            # Log only if status actually changes (might already be empty)",
        "            if st.session_state.status_texto[c]: log_status_change(c, old_status, '', duration)",
        "            st.session_state.status_texto[c] = ''",
        "            changed = True",
        "",
        "    # Assign baton if we found someone and they don't have it yet",
        ...
        "",
        "    if is_checked:",
        "        log_status_change(consultor, old_status, '', duration) # Log becoming available",
        ...
        "    # Always check baton assignment, especially if holder left or cycle reset",
        "    baton_changed = check_and_assume_baton()",
        "",
        "    # Save state if reset or baton changed (already saved in check_and_assume if baton changed)",
        "    if reset_triggered and not baton_changed: save_state()",
        "    elif not reset_triggered and not baton_changed: save_state() # Save if only queue changed",
        "",
        "    st.rerun()",
        "",
        "def rotate_bastao():",
        ...
        "",
        "    # Save state (already saved in check_and_assume)",
        "    # save_state() # Redundant if check_and_assume saves",
        "    st.rerun()",
        "",
        "def skip_turn():",
        "    print('SKIP TURN')",
        ...
        "    # Do NOT add to completed_this_lap when skipping",
        "",
        "    print(f' Fila Ativa: {st.session_state.bastao_queue}')",
        "    print(f' Master Order: {st.session_state.master_order}')",
        "    print(f' Skipped: {st.session_state.skipped_this_lap}')",
        ...
        "        baton_changed = check_and_assume_baton()",
        "",
        "    # Save state if reset or baton changed (already saved if baton changed)",
        "    if reset_triggered and not baton_changed: save_state()",
        "    elif not reset_triggered and not baton_changed: save_state() # Save if only queue changed",
        "",
        "    st.rerun()",
        ...
        "# --- INÍCIO DA APLICAÇÃO ---",
        "",
        "init_session_state() # Load state and set defaults",
        "",
        ...
        "        st.rerun() # Rerun if baton was assigned",
        "",
        "# --- Layout Principal ---",
