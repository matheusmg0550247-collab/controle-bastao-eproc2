...
def get_global_state_cache():
    ...
    return {
        ...
        'status_transitions': [], # <-- Lista para armazenar transições
    }
...
def save_state():
    ...
    try:
        ...
        global_data['status_transitions'] = st.session_state.status_transitions.copy() # Salva transições
...
def send_chat_notification_internal(consultor, status):
    ...
    if CHAT_WEBHOOK_BASTAO and status == 'Bastão':
        ...
        except requests.exceptions.RequestException as e: print(f"Erro: {e}"); return False
    return False
...
def log_status_change(consultor, old_status, new_status, duration):
    ...
    if 'status_transitions' not in st.session_state: st.session_state.status_transitions = []
    
    transition_entry = {
        'timestamp': datetime.now(),
        'consultor': consultor,
        'previous_status': old_status if old_status is not None else 'Inicio',
        'current_status': new_status if new_status is not None else 'Disponível',
    ...
    save_state() # Salva imediatamente

def format_time_duration(duration_obj):
    """Formata um objeto timedelta ou um número de segundos."""
    if isinstance(duration_obj, timedelta): s = int(duration_obj.total_seconds())
    elif isinstance(duration_obj, (int, float)): s = int(duration_obj)
    else: return '--:--:--'
    if s < 0: s = 0 
    h, remainder = divmod(s, 3600); m, s = divmod(remainder, 60)
    return f'{h:02}:{m:02}:{s:02}'

# <-- MODIFICADO: Função send_daily_report atualizada -->
def send_daily_report():
    ...
    if not today_transitions:
        ...
        for t in today_transitions:
            ...
            # else: # Comentado para não poluir o log se um status inesperado aparecer
            #      print(f"Aviso: Consultor '{consultor}' ou status '{report_status}' não encontrado.")
    ...
    if not today_transitions:
        ...
        for consultor in CONSULTORES:
            ...
            for status in ALL_STATUSES_FOR_REPORT:
                 ...
                 if display_status == 'Inicio': continue 
    ...
    try:
        ...
        if e.response is not None: print(f'Status: {e.response.status_code}, Resposta: {e.response.text}')
...
def init_session_state():
    ...
    for key, default_value in defaults.items():
        value_from_global = persisted_state.get(key)
        if key in ['play_sound', 'gif_warning']:
             st.session_state.setdefault(key, default_value)
             continue 
        if value_from_global is not None:
            if isinstance(value_from_global, (list, dict)): st.session_state[key] = value_from_global.copy()
            else: st.session_state[key] = value_from_global
        else:
             if isinstance(default_value, (list, dict)): st.session_state[key] = default_value.copy()
...
def find_next_holder_index(current_index, queue, skips):
    ...
    next_idx = (current_index + 1) % num_consultores; attempts = 0
    while attempts < num_consultores:
        ...
        next_idx = (next_idx + 1) % num_consultores; attempts += 1
    print("AVISO: find_next_holder_index não encontrou ninguém elegível."); return -1

def check_and_assume_baton():
    # (Código mantido, toca som)
    ...
    should_have_baton = first_eligible_holder if not is_current_valid else current_holder_status
    ...
    for c in CONSULTORES:
        if c != should_have_baton and st.session_state.status_texto.get(c) == 'Bastão':
            print(f'Limpando bastão de {c}'); duration = datetime.now() - st.session_state.current_status_starts.get(c, datetime.now())
            log_status_change(c, 'Bastão', 'Indisponível', duration); st.session_state.status_texto[c] = 'Indisponível'; changed = True
    if should_have_baton and st.session_state.status_texto.get(should_have_baton) != 'Bastão':
        print(f'Atribuindo bastão para {should_have_baton}'); old_status = st.session_state.status_texto.get(should_have_baton, '')
        duration = datetime.now() - st.session_state.current_status_starts.get(should_have_baton, datetime.now())
        log_status_change(should_have_baton, old_status, 'Bastão', duration); st.session_state.status_texto[should_have_baton] = 'Bastão'
        st.session_state.bastao_start_time = datetime.now()
        if previous_holder != should_have_baton: 
            st.session_state.play_sound = True; print("SOUND TRIGGER: check_and_assume_baton assigned baton."); send_chat_notification_internal(should_have_baton, 'Bastão') 
        if st.session_state.skip_flags.get(should_have_baton): print(f' Consumindo skip flag de {should_have_baton}'); st.session_state.skip_flags[should_have_baton] = False
        changed = True
    elif not should_have_baton:
        if current_holder_status:
            print(f'Ninguém elegível, limpando bastão de {current_holder_status}'); duration = datetime.now() - st.session_state.current_status_starts.get(current_holder_status, datetime.now())
            log_status_change(current_holder_status, 'Bastão', 'Indisponível', duration); st.session_state.status_texto[current_holder_status] = 'Indisponível' ; changed = True
        if st.session_state.bastao_start_time is not None: st.session_state.bastao_start_time = None; changed = True; save_state() # Salva se só limpou start_time
    # if changed: save_state() # log_status_change já salva
    return changed
...
def update_queue(consultor):
    ...
    if is_checked: 
        log_status_change(consultor, old_status_text or 'Indisponível', '', duration) ; st.session_state.status_texto[consultor] = '' 
        if consultor not in st.session_state.bastao_queue: st.session_state.bastao_queue.append(consultor) ; print(f'Add {consultor}')
        ...
        log_status_change(consultor, log_old_status , 'Indisponível', duration) ; st.session_state.status_texto[consultor] = 'Indisponível' 
        if consultor in st.session_state.bastao_queue: st.session_state.bastao_queue.remove(consultor); print(f'Rem {consultor}')
        st.session_state.skip_flags.pop(consultor, None) 
    baton_changed = check_and_assume_baton() 
    # save_state() é chamado em log_status_change e check_and_assume_baton

def rotate_bastao(): 
    # (Código mantido, toca som diretamente)
    print('CALLBACK ROTATE BASTAO (PASSAR)'); selected = st.session_state.consultor_selectbox
    st.session_state.gif_warning = False; st.session_state.rotation_gif_start_time = None; st.session_state.lunch_warning_info = None 
    ...
    except ValueError: st.warning(f'Erro: {current_holder} not in queue.'); check_and_assume_baton(); return
    reset_triggered = False; first_eligible_index_overall = find_next_holder_index(-1, queue, skips) 
    ...
    if potential_next_index != -1 and first_eligible_index_overall != -1:
        first_eligible_holder_overall = queue[first_eligible_index_overall]; potential_next_holder = queue[potential_next_index]
        if potential_next_holder == first_eligible_holder_overall and current_holder != first_eligible_holder_overall:
            print("--- RESETANDO CICLO ---"); st.session_state.skip_flags = {c: False for c in CONSULTORES if st.session_state.get(f'check_{c}')}; skips = st.session_state.skip_flags 
            reset_triggered = True; next_index = first_eligible_index_overall 
        else: next_index = potential_next_index
    else: next_index = -1
    if next_index != -1:
        next_holder = queue[next_index]; print(f'Passando de {current_holder} para {next_holder} (Reset: {reset_triggered})')
        ...
        log_status_change(next_holder, st.session_state.status_texto.get(next_holder, ''), 'Bastão', timedelta(0)); st.session_state.status_texto[next_holder] = 'Bastão'
        st.session_state.bastao_start_time = datetime.now(); st.session_state.skip_flags[next_holder] = False 
        st.session_state.bastao_counts[current_holder] = st.session_state.bastao_counts.get(current_holder, 0) + 1
        st.session_state.play_sound = True; print("SOUND TRIGGER: rotate_bastao successful."); st.session_state.rotation_gif_start_time = datetime.now()
        # save_state() # log_status_change já salva
    else: print('Ninguém elegível.'); st.warning('Não há próximo consultor elegível.'); check_and_assume_baton() 

def toggle_skip(): 
    # (Código mantido, sem st.rerun())
    print('CALLBACK TOGGLE SKIP'); selected = st.session_state.consultor_selectbox
    st.session_state.gif_warning = False; st.session_state.rotation_gif_start_time = None; st.session_state.lunch_warning_info = None 
    if not selected or selected == 'Selecione um nome': st.warning('Selecione um consultor.'); return
    if not st.session_state.get(f'check_{selected}'): st.warning(f'{selected} não disponível.'); return
    current_skip_status = st.session_state.skip_flags.get(selected, False); st.session_state.skip_flags[selected] = not current_skip_status
    new_status_str = 'MARCADO' if not current_skip_status else 'DESMARCADO'; print(f'{selected} {new_status_str}')
    current_holder = next((c for c, s in st.session_state.status_texto.items() if s == 'Bastão'), None)
    if selected == current_holder and st.session_state.skip_flags[selected]: print(f'Portador {selected} pulou.'); save_state(); rotate_bastao(); return 
    save_state() 

# <-- MODIFICADO: Mensagem de almoço simplificada -->
def update_status(status_text, change_to_available): 
    print(f'CALLBACK UPDATE STATUS: {status_text}'); selected = st.session_state.consultor_selectbox
    ...
    current_lunch_warning = st.session_state.get('lunch_warning_info'); is_second_try = False
    ...
    if status_text == 'Almoço' and not is_second_try:
        all_statuses = st.session_state.status_texto; num_na_fila = sum(1 for s in all_statuses.values() if s == '' or s == 'Bastão')
        num_atividade = sum(1 for s in all_statuses.values() if s == 'Atividade'); total_ativos = num_na_fila + num_atividade
        num_almoco = sum(1 for s in all_statuses.values() if s == 'Almoço'); limite_almoco = total_ativos / 2.0
        ...
        if total_ativos > 0 and num_almoco >= limite_almoco:
            ...
            st.session_state.lunch_warning_info = {
                'consultor': selected, 'start_time': datetime.now(),
    ...
    print(f'... Fila: {st.session_state.bastao_queue}, Skips: {st.session_state.skip_flags}'); baton_changed = False
    if was_holder: baton_changed = check_and_assume_baton() 
    # if not baton_changed: save_state() # log_status_change já salva
...
show_gif = False; show_lunch_warning = False; refresh_interval = 40000 
...
if lunch_warning_info and lunch_warning_info.get('start_time'):
    ...
    except Exception as e: print(f"Erro timer almoço: {e}"); st.session_state.lunch_warning_info = None
            
st_autorefresh(interval=refresh_interval, key='auto_rerun_key') 

# Lógica de som simplificada
...
if st.session_state.get('gif_warning', False): st.error('🚫 Ação inválida!'); st.image(GIF_URL_WARNING, width=150)
...
proximo = queue[proximo_index] if proximo_index != -1 else None; restante = []
if proximo_index != -1: 
    num_q = len(queue); start_check_idx = (proximo_index + 1) % num_q; current_check_idx = start_check_idx; checked_count = 0
    while checked_count < num_q:
        ...
        if 0 <= current_check_idx < num_q:
            ...
            if consultor != responsavel and consultor != proximo and not skips.get(consultor, False) and st.session_state.get(f'check_{consultor}'): restante.append(consultor)
        current_check_idx = (current_check_idx + 1) % num_q; checked_count += 1

# --- Coluna Principal ---
with col_principal:
    # (Código mantido)
    st.header("Responsável pelo Bastão"); _, col_time = st.columns([0.25, 0.75]); duration = timedelta()
    ...
    if responsavel: st.markdown(f'<span style="font-size: 2em; font-weight: bold;">{responsavel}</span>', unsafe_allow_html=True)
    ...
    if not proximo and not restante:
        ...
        elif queue and all(skips.get(c, False) or not st.session_state.get(f'check_{c}') for c in queue) : st.markdown('*Todos pulando...*')
        else: st.markdown('*Ninguém elegível.*')
    ...
    if skipped_consultants:
        skipped_text = ', '.join(sorted(skipped_consultants)); num_skipped = len(skipped_consultants)
        titulo = '**Pulou:**' if num_skipped == 1 else '**Pularam:**'; verbo_pular = 'pulou' if num_skipped == 1 else 'pularam'; verbo_retornar = 'Retorna' if num_skipped == 1 else 'Retornam'
        st.markdown(f'''<div style="margin-top: 15px;"><span style="color: #FFC107; font-weight: bold;">{titulo}</span><br><span style="color: black;">{skipped_text} {verbo_pular}!</span><br><span style="color: black;">{verbo_retornar} no próximo ciclo!</span></div>''', unsafe_allow_html=True)
    st.markdown("###"); st.header("**Consultor**"); st.selectbox('Selecione:', options=['Selecione um nome'] + CONSULTORES, key='consultor_selectbox', label_visibility='collapsed')
    st.markdown("#### "); st.markdown("**Ações:**"); c1, c2, c3, c4, c5, c6, c7 = st.columns(7) 
    c1.button('🎯 Passar', on_click=rotate_bastao, use_container_width=True, help='Passa bastão.'); c2.button('⏭️ Pular', on_click=toggle_skip, use_container_width=True, help='Marca/Desmarca pular.')
    c3.button('✏️ Atividade', on_click=update_status, args=('Atividade', False,), use_container_width=True); c4.button('🍽️ Almoço', on_click=update_status, args=('Almoço', False,), use_container_width=True)
    c5.button('👤 Ausente', on_click=update_status, args=('Ausente', False,), use_container_width=True); c6.button('🎙️ Sessão', on_click=update_status, args=('Sessão', False,), use_container_width=True)
...
with col_disponibilidade:
    # (Código mantido)
    st.header('Status dos Consultores'); st.markdown('Marque/Desmarque para entrar/sair.')
    ui_lists = {'fila': [], 'atividade': [], 'almoco': [], 'saida': [], 'ausente': [], 'sessao': [], 'indisponivel': []}
    for nome in CONSULTORES:
        is_checked = st.session_state.get(f'check_{nome}', False); status = st.session_state.status_texto.get(nome, 'Indisponível')
        ...
        elif status == 'Sessão': ui_lists['sessao'].append(nome)
    ...
    if not render_order: st.markdown('_Ninguém disponível._')
        ...
        for nome in render_order:
            col_nome, col_check = st.columns([0.8, 0.2]); key = f'check_{nome}'
            col_check.checkbox(' ', key=key, on_change=update_queue, args=(nome,), label_visibility='collapsed')
            skip_flag = skips.get(nome, False)
            if nome == responsavel: display = f'<span style="background-color: #E75480; color: white; padding: 2px 6px; border-radius: 5px; font-weight: bold;">🔥 {nome}</span>'
    ...
    def render_section(title, icon, names, tag_color):
        ...
        st.markdown('---')
    render_section('Atividade', '✏️', ui_lists['atividade'], 'yellow'); render_section('Almoço', '🍽️', ui_lists['almoco'], 'blue')
    render_section('Ausente', '👤', ui_lists['ausente'], 'violet'); render_section('Sessão', '🎙️', ui_lists['sessao'], 'green')
    render_section('Saída', '🚶', ui_lists['saida'], 'red'); render_section('Indisponível', '❌', ui_lists['indisponivel'], 'grey')
    now = datetime.now(); last_run_dt = st.session_state.report_last_run_date
    if not isinstance(last_run_dt, datetime): last_run_dt = datetime.min
    if now.hour >= 20 and now.date() > last_run_dt.date(): send_daily_report()
