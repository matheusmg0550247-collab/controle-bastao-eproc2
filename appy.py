...
import pandas as pd # Mantido, embora não usado
import requests
...
# ... (save_state, load_state, send_chat_notification_internal, play_sound_html, load_logs, save_logs - iguais ao código anterior) ...
# ============================================
...
def save_state():
    ...
    try:
        ...
        global_data['status_transitions'] = st.session_state.status_transitions.copy() 
...
# <-- MODIFICADO: Função init_session_state refatorada -->
def init_session_state():
    """Inicializa/sincroniza o st.session_state com o estado GLOBAL do cache."""
    persisted_state = load_state()
    
    # Define os padrões para TODAS as chaves esperadas no session_state
    defaults = {
        'status_texto': {nome: 'Indisponível' for nome in CONSULTORES},
        'bastao_queue': [],
        'skip_flags': {},
    ...
    # Carrega CADA chave do estado persistido OU usa o default
    # Garante que listas e dicionários sejam copiados
    for key, default_value in defaults.items():
        value_from_global = persisted_state.get(key)
        
        # Mantém flags locais se já existirem na sessão atual
        if key in ['play_sound', 'gif_warning']:
             st.session_state.setdefault(key, default_value)
             continue # Pula para a próxima chave

        # Carrega do global ou usa default
        if value_from_global is not None:
            if isinstance(value_from_global, (list, dict)):
                st.session_state[key] = value_from_global.copy()
            else:
                st.session_state[key] = value_from_global
        else:
             # Se não veio do global, usa o default (copiando se for mutável)
             if isinstance(default_value, (list, dict)):
                 st.session_state[key] = default_value.copy()
             else:
                 st.session_state[key] = default_value

    # Garante que todos os consultores existem nas estruturas Dicionário
    # (Após elas terem sido carregadas/inicializadas)
    for nome in CONSULTORES:
        ...
        # Sincroniza o checkbox baseado no status carregado
    ...
    # Lógica de reconstrução de fila (após carregar a fila)
    ...
    # Chama a verificação do bastão DEPOIS que tudo foi carregado
...
def check_and_assume_baton():
    # (igual ao código anterior, já toca som)
    ...
    for c in CONSULTORES:
        if c != should_have_baton and st.session_state.status_texto.get(c) == 'Bastão':
            ...
            # st.session_state.status_texto[c] = 'Indisponível' # log_status_change já salva
            changed = True
    if should_have_baton and st.session_state.status_texto.get(should_have_baton) != 'Bastão':
        ...
        # st.session_state.status_texto[should_have_baton] = 'Bastão' # log_status_change já salva
        ...
        if st.session_state.skip_flags.get(should_have_baton):
            ...
            # save_state() # Não precisa, log_status_change já salvou
        changed = True
        ...
        if current_holder_status:
            ...
            # st.session_state.status_texto[current_holder_status] = 'Indisponível' # log_status_change já salva
            changed = True
        if st.session_state.bastao_start_time is not None: 
             st.session_state.bastao_start_time = None
             changed = True # Indica que o estado precisa ser salvo
             save_state() # Salva explicitamente se só limpou o start_time

    # Não precisa salvar aqui, pois log_status_change já salva quando muda
    # if changed: save_state() 
    return changed


def update_queue(consultor):
    # (igual ao código anterior, sem st.rerun())
...
def rotate_bastao(): 
    # (igual ao código anterior, toca som diretamente)
    ...
    if next_index != -1:
        ...
        # st.session_state.status_texto[current_holder] = '' # log já salva
        log_status_change(next_holder, st.session_state.status_texto.get(next_holder, ''), 'Bastão', timedelta(0)) 
        # st.session_state.status_texto[next_holder] = 'Bastão' # log já salva
        st.session_state.bastao_start_time = datetime.now()
        ...
        # save_state() # log já salva
    else:
        print('Ninguém elegível.')
...
def toggle_skip(): 
    ...
    if selected == current_holder and st.session_state.skip_flags[selected]:
        ...
        save_state() # Salva a flag antes de chamar rotate
        rotate_bastao() 
        return 
    save_state() 

def update_status(status_text, change_to_available): 
    # (igual ao código anterior, chama log_status_change)
    ...
    st.session_state.status_texto[selected] = status_text # <-- Definir status DEPOIS de logar
    ...
    if was_holder: 
        baton_changed = check_and_assume_baton() 
    # save_state já é chamado em log_status_change e check_and_assume_baton
