...
# 🛑 WEBHOOK DE RELATÓRIO DIÁRIO (DESABILITADO)
...
# ⬇️ WEBHOOK DE NOTIFICAÇÃO IMEDIATA (DESABILITADO)
...
def generate_app_code(consultores, emoji, webhook_relatorio, webhook_bastao, public_url):
    ...
    app_code_lines = [
        ...
        "init_session_state()", 
        "",
        "# --- ⬇️ MUDANÇA AQUI (Lógica de Refresh Unificada) ⬇️ ---",
        "# st_autorefresh(interval=30000, key='auto_rerun_key') # <-- REMOVIDO DAQUI",
        "",
        ...
        "show_gif = False",
        "refresh_interval = 30000 # Padrão de 30s",
        "",
        "if gif_start_time:",
        "    try:",
        ...
        "        if time_elapsed < 20:",
        "            show_gif = True",
        "            refresh_interval = 5000 # Override para 5s enquanto o GIF estiver ativo",
        "        else:",
        "            st.session_state['rotation_gif_start_time'] = None # Limpa o GIF",
        "            show_gif = False",
        "    except TypeError:",
        "        st.session_state['rotation_gif_start_time'] = None",
        "        show_gif = False",
        "",
        "# Renderiza o auto-refresh UMA VEZ com o intervalo dinâmico",
        ...
        "# --- ⬆️ FIM DA MUDANÇA (Lógica de Refresh Unificada) ⬆️ ---",
        "",
        ...
        "    # 2. Renderizar seções",
        "",
        "    # --- SEÇÃO BASTÃO E FILA (DISPONÍVEIS) ---",
        "    st.subheader(f'✅ Na Fila ({len(list_na_fila_ordenada)})')",
        ...
        "",
        "    # --- SEÇÃO ATIVIDADES ---",
        "    st.subheader(f'✏️ Atividade ({len(list_atividade)})')",
        "    if not list_atividade: st.markdown('_Ninguém em atividade._')",
        ...
        "",
        "    # --- SEÇÃO ALMOÇO ---",
        "    st.subheader(f'🍽️ Almoço ({len(list_almoco)})')",
        "    if not list_almoco: st.markdown('_Ninguém em almoço._')",
        "    for nome in list_almoco:",
        "        col_nome, col_check = st.columns([0.8, 0.2])",
        "        checkbox_key = f'check_{nome}'",
        "        is_disabled = nome in st.session_state['skipped_consultors']",
        "        col_check.checkbox(' ', key=checkbox_key, on_change=update_queue, args=(nome,), disabled=is_disabled, label_visibility='collapsed')",
        "        col_nome.markdown(f'**{nome}** :blue-background[Almoço]', unsafe_allow_html=True)",
        "",
        "    st.markdown('---')",
        "",
        "    # --- SEÇÃO SAÍDA ---",
        "    st.subheader(f'🚶 Saída ({len(list_saida)})')",
        "    if not list_saida: st.markdown('_Ninguém em saída._')",
        "    for nome in list_saida:",
        "        col_nome, col_check = st.columns([0.8, 0.2])",
        "        checkbox_key = f'check_{nome}'",
        "        is_disabled = nome in st.session_state['skipped_consultors']",
        "        col_check.checkbox(' ', key=checkbox_key, on_change=update_queue, args=(nome,), disabled=is_disabled, label_visibility='collapsed')",
        "        col_nome.markdown(f'**{nome}** :red-background[Saída]', unsafe_allow_html=True)",
        "",
        "    st.markdown('---')",
        "",
        "    # --- SEÇÃO INDISPONÍVEL ---",
