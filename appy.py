...
def generate_app_code(emoji, webhook_relatorio, webhook_bastao, public_url):
    consultores_list_str = str(CONSULTORES) # Define CONSULTORES inside
    ...
    app_code_lines = [
        ...
        "from streamlit_autorefresh import st_autorefresh",
        "",
        f"CONSULTORES = {consultores_list_str}", # Definido aqui
        ...
        "        'bastao_queue': st.session_state.bastao_queue,", # Fila única agora
        ...
        "        'master_order': st.session_state.master_order,", # Ainda usamos para ordem
        "        # completed_this_cycle não é persistido",
        "    }",
        "    try:",
        ...
        "                 if c in CONSULTORES:", # Only load for current consultants
        ...
        "        data['master_order'] = [c for c in data.get('master_order', []) if c in CONSULTORES]", # Clean loaded master order
        "        data.setdefault('status_texto', {})",
        "        data.setdefault('bastao_queue', [])",
        "        data['bastao_queue'] = [c for c in data.get('bastao_queue', []) if c in CONSULTORES]", # Clean loaded queue
        ...
        "        'master_order': [],",
        "        'completed_this_cycle': set() # Sempre reseta",
        "    }",
        "    for key, default in defaults.items():",
        "        if key == 'completed_this_cycle': st.session_state[key] = default", # Reset non-persistent
        "        else: st.session_state.setdefault(key, persisted_state.get(key, default))",
        "        # Type safety",
        "        if key == 'completed_this_cycle' and not isinstance(st.session_state[key], set): st.session_state[key] = set()",
        ...
        "        # Set default only if key doesn't exist, otherwise keep current checkbox value",
        "        st.session_state.setdefault(f'check_{nome}', is_active)",
        "",
        "    # Se a fila está vazia mas deveria ter gente (baseado nos checks), reconstruir",
        ...
        "        st.session_state.bastao_queue = [c for c in st.session_state.master_order if c in checked_on]",
        "        for c in checked_on:",
        ...
        "        st.session_state.completed_this_cycle = set() # Start fresh cycle",
        "",
        "    print('--- Estado Inicializado ---')",
        "    print(f' Master: {st.session_state.master_order}, Fila: {st.session_state.bastao_queue}, Completed: {st.session_state.completed_this_cycle}')",
        "",
        ...
        "    # Quem está disponível agora e faz parte da ordem mestre?",
        "    available_in_master = {c for c in master if st.session_state.get(f'check_{c}')}",
        "    print(f'CHECK RESET: Disponíveis na Master: {available_in_master}, Completaram: {completed}')",
        "    # Reseta se o conjunto de disponíveis na master estiver contido nos que completaram",
        "    if available_in_master and available_in_master.issubset(completed):",
        "        print('--- RESETANDO CICLO (Limpando Completed) ---')",
        "        st.session_state.completed_this_cycle = set()",
        "        # A fila (bastao_queue) não precisa ser reconstruída aqui, check_and_assume_baton vai achar o primeiro",
        "        return True",
        "    return False",
        "",
        ...
        "    # Próximo é o primeiro na fila que NÃO completou E está disponível",
        "    next_holder = next((c for c in queue if c not in completed and st.session_state.get(f'check_{c}')), None)",
        "    print(f'Fila: {queue}, Completed: {completed}, Próximo: {next_holder}, Atual Portador: {current_holder_status}')",
        "",
        ...
        "             # Adiciona na posição correta da master order, se possível",
        "             current_available = {c for c in st.session_state.bastao_queue if st.session_state.get(f'check_{c}')}",
        "             inserted = False",
        "             new_queue = []",
        "             consultor_added = False",
        "             for master_c in st.session_state.master_order:",
        "                 if master_c == consultor:",
        "                      new_queue.append(consultor)",
        "                      consultor_added = True",
        "                 elif master_c in current_available:", # Add others who are still available
        "                      new_queue.append(master_c)",
        "             if not consultor_added: new_queue.append(consultor) # Add at end if not in master order somehow",
        "             st.session_state.bastao_queue = new_queue",
        "        # Não mexe em completed_this_cycle aqui",
        "    else:",
        "        log_status_change(consultor, old_status_text or ('Bastão' if was_holder else 'Disponível'), 'Indisponível', duration)",
        "        st.session_state.status_texto[consultor] = ''",
        "        # Remove da fila ativa (bastao_queue), mas mantém na master_order",
        "        if consultor in st.session_state.bastao_queue: st.session_state.bastao_queue.remove(consultor)",
        "        st.session_state.completed_this_cycle.discard(consultor)",
        "",
        "    print(f'... Fila Ativa: {st.session_state.bastao_queue}, Master: {st.session_state.master_order}, Completed: {st.session_state.completed_this_cycle}')",
        "    reset = check_cycle_reset()",
        "    baton = check_and_assume_baton()",
        "    if not baton: save_state()", # Salva se check_assume não salvou
        "    st.rerun()",
        "",
        ...
        "    # NÃO remove da bastao_queue, apenas marca como completo",
        ...
        "    print(f'... Fila Ativa: {st.session_state.bastao_queue}, Master: {st.session_state.master_order}, Completed: {st.session_state.completed_this_cycle}')",
        "    reset = check_cycle_reset()",
        "    check_and_assume_baton()", # Acha o próximo NÃO completo ou reseta
        ...
        "    # Use a flag to prevent potential infinite loops if assignment fails",
        ...
        "active_queue = st.session_state.bastao_queue", # Usamos bastao_queue como base
        "master = st.session_state.master_order",
        "completed = st.session_state.completed_this_cycle",
        ...
        "# Restante: todos na fila DEPOIS do próximo que não completaram E estão disponíveis",
        "proximo_index = -1",
        "if proximo and proximo in active_queue:",
        "   try: proximo_index = active_queue.index(proximo)",
        "   except ValueError: pass",
        "restante = [c for i, c in enumerate(active_queue) if proximo_index != -1 and i > proximo_index and c not in completed and st.session_state.get(f'check_{c}')]",
        "",
        "with col_principal:",
        ...
        "         elif active_queue and all((c in completed or not st.session_state.get(f'check_{c}')) for c in active_queue) : st.markdown('*Todos disponíveis completaram. Aguardando reset do ciclo...*')",
        ...
        "    c1.button('🎯 Bastão', on_click=finish_turn_action, use_container_width=True, help='Finaliza seu turno e passa.')",
        "    c2.button('⏭️ Pular', on_click=finish_turn_action, use_container_width=True, help='Finaliza seu turno e passa.')", # Mesma ação
        ...
        "    # Ordena a exibição pela master_order, mostrando apenas os disponíveis (checked)",
        "    render_order = [c for c in master if c in ui_lists['fila']] + [c for c in ui_lists['fila'] if c not in master]",
        "    if not render_order: st.markdown('_Ninguém disponível._')",
        "    else:",
        "        for nome in render_order:",
        "            col_nome, col_check = st.columns([0.8, 0.2])",
        "            key = f'check_{nome}'",
        "            # Usar on_change aqui parece mais estável para checkboxes",
        ...
        "            else: # Está na fila, disponível, não completou e não é o responsável => Aguardando",
    ...
    # Limpa linhas vazias extras que podem ter sido introduzidas
    clean_lines = [line for line in app_code_lines if line.strip()]
    return "\n".join(clean_lines) + "\n"
