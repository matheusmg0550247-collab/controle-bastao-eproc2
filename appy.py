...
def generate_app_code(emoji, webhook_relatorio, webhook_bastao, public_url):
    ...
    app_code_lines = [
        ...
        "    if current_index >= num_consultores or current_index < -1: current_index = -1 # Safety check",
        "",
        ...
        "        st.session_state.bastao_start_time = datetime.now()",
        "        st.session_state.skip_flags[next_holder] = False # Consome flag",
        ...
        "    st.session_state.gif_warning = False; st.session_state.rotation_gif_start_time = None",
        "    if not selected or selected == 'Selecione um nome': st.warning('Selecione um consultor.'); return",
        "    # Permite marcar/desmarcar qualquer um que esteja checado (na fila implícita)",
        ...
        "",
        "    # Se o portador atual se marcou para pular, tenta passar imediatamente",
        "    current_holder = next((c for c, s in st.session_state.status_texto.items() if s == 'Bastão'), None)",
        ...
        "        rotate_bastao() # Chama a função de passar (que salva e faz rerun)",
        "        return",
        "",
        "    save_state()",
        "    st.rerun()",
        ...
        "            st.rerun()",
        "    else: print('AVISO: Segunda tentativa de assumir bastão no render evitada.')",
        ...
        "    count = 0",
        "    while count < num_q:",
        "        if idx == current_index or idx == proximo_index: # Pula atual e próximo",
        "            idx = (idx + 1) % num_q",
        "            count += 1",
...
# 5. EXECUÇÃO FINAL
# ============================================
# Define as funções no escopo global ANTES de chamar exec
# (Isso garante que elas existam quando o código dentro do exec for executado)
date_serializer = date_serializer
save_state = save_state
load_state = load_state
send_chat_notification_internal = send_chat_notification_internal
play_sound_html = play_sound_html
load_logs = load_logs
save_logs = save_logs
...
find_next_holder_index = find_next_holder_index
check_and_assume_baton = check_and_assume_baton
update_queue = update_queue
rotate_bastao = rotate_bastao
toggle_skip = toggle_skip
update_status = update_status
manual_rerun = manual_rerun

# Gera a string contendo apenas a lógica principal do app
